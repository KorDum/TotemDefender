...
public class WorldGenTotemTree extends WorldGenAbstractTree {
    private static final IBlockState TRUNK = ModBlocks.log.getDefaultState();
    private static final IBlockState TRUNK_FACE1 = ModBlocks.logFace1.getDefaultState();
    private static final IBlockState TRUNK_FACE2 = ModBlocks.logFace2.getDefaultState();
    private static final IBlockState TRUNK_FACE3 = ModBlocks.logFace3.getDefaultState();

    private static final IBlockState LEAF = ModBlocks.leaves.getDefaultState()
    ...
    public boolean generate(World world, Random rand, BlockPos position) {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= world.getHeight()) {
                ...
                for (int k1 = position.getX() - j1; k1 <= position.getX() + j1 && flag; ++k1) {
                    for (int l1 = position.getZ() - j1; l1 <= position.getZ() + j1 && flag; ++l1) {
                        if (i1 >= 0 && i1 < world.getHeight()) {
                            ...
                            if (!state.getBlock().isAir(state, world, blockpos$mutableblockpos.setPos(k1, i1, l1))
                                && !state.getBlock().isLeaves(state, world, blockpos$mutableblockpos.setPos(k1, i1, l1))) {
                                flag = false;
                            }
                        } else {
                            flag = false;
            ...
            if (state.getBlock().canSustainPlant(state, world, down, net.minecraft.util.EnumFacing.UP, ModBlocks.sapling) && position.getY() < world.getHeight() - i - 1) {
